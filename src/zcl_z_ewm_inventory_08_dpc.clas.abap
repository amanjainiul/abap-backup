class ZCL_Z_EWM_INVENTORY_08_DPC definition
  public
...
protected section.
  ...
  methods INVENTORYADJUSTM_CREATE_ENTITY
    ...
    exporting
      !ER_ENTITY type ZCL_Z_EWM_INVENTORY_08_MPC=>TS_INVENTORYADJUSTMENTITEM
    raising
  ...
  methods INVENTORYADJUSTM_GET_ENTITYSET
    ...
    exporting
      !ET_ENTITYSET type ZCL_Z_EWM_INVENTORY_08_MPC=>TT_INVENTORYADJUSTMENTITEM
...
CLASS ZCL_Z_EWM_INVENTORY_08_DPC IMPLEMENTATION.


  method /IWBEP/IF_MGW_APPL_SRV_RUNTIME~CREATE_ENTITY.
*&----------------------------------------------------------------------------------------------*
*&  Include           /IWBEP/DPC_TEMP_CRT_ENTITY_BASE
*&* This class has been generated on 18.09.2025 13:12:26 in client 100
*&*
*&*       WARNING--> NEVER MODIFY THIS CLASS <--WARNING
*&*   If you want to change the DPC implementation, use the
*&*   generated methods inside the DPC provider subclass - ZCL_Z_EWM_INVENTORY_08_DPC_EXT
*&-----------------------------------------------------------------------------------------------*

 DATA inventoryadjustm_create_entity TYPE zcl_z_ewm_inventory_08_mpc=>ts_inventoryadjustmentitem.
...
*&-----------------------------------------------------------------------------------------------*
 ...
 DATA inventoryadjustm_get_entity TYPE zcl_z_ewm_inventory_08_mpc=>ts_inventoryadjustmentitem.
 DATA lv_entityset_name TYPE string.
...
*&*   generated methods inside the DPC provider subclass - ZCL_Z_EWM_INVENTORY_08_DPC_EXT
*&-----------------------------------------------------------------------------------------------*
 DATA inventoryadjustm_get_entityset TYPE zcl_z_ewm_inventory_08_mpc=>tt_inventoryadjustmentitem.
 DATA lv_entityset_name TYPE string.
...
*&-----------------------------------------------------------------------------------------------*
 ...
 DATA inventoryadjustm_update_entity TYPE zcl_z_ewm_inventory_08_mpc=>ts_inventoryadjustmentitem.
 DATA lv_entityset_name TYPE string.
 DATA lr_entity TYPE REF TO data. "#EC NEEDED
...
METHOD inventoryadjustm_create_entity.
  ...
  DATA: ls_input     TYPE zcl_z_ewm_inventory_02_mpc=>ts_inventoryadjustmentitem,
...
*--------------------------------------------------------------------
  IF ls_input-matnr IS NOT INITIAL.
    CALL FUNCTION 'CONVERSION_EXIT_MDLPD_INPUT'
      EXPORTING input = ls_input-matnr
      IMPORTING output = lv_matid
      EXCEPTIONS OTHERS = 1.
  ENDIF.
  ...
  IF ls_input-charg IS NOT INITIAL AND lv_matid IS NOT INITIAL.
    ...
    TRY.
        CALL METHOD /scwm/cl_batch_appl=>get_batchid_by_no
          EXPORTING iv_lgnum       = ls_input-lgnum
                    is_matid_charg = ls_mat_charg
          IMPORTING ev_batchid     = lv_batchid.
...
*--------------------------------------------------------------------
  ...
  <ls_cnt>-count_user   = ls_input-uname.   " <-- FIXED: user from payload
  <ls_cnt>-reason       = 'UPLD'.
...
*--------------------------------------------------------------------
  ...
  er_entity-processor     = ls_input-uname.
  er_entity-vlenr     = ls_input-vlenr.
  er_entity-matnr     = ls_input-matnr.
  er_entity-count_qty    = ls_input-cntqty.
