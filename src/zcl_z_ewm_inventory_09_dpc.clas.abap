class ZCL_Z_EWM_INVENTORY_09_DPC definition
  public
...
protected section.
  ...
  methods INVENTORYADJUSTM_CREATE_ENTITY
    ...
    exporting
      !ER_ENTITY type ZCL_Z_EWM_INVENTORY_09_MPC=>TS_INVENTORYADJUSTMENTITEMN
    raising
  ...
  methods INVENTORYADJUSTM_GET_ENTITYSET
    ...
    exporting
      !ET_ENTITYSET type ZCL_Z_EWM_INVENTORY_09_MPC=>TT_INVENTORYADJUSTMENTITEMN
...
CLASS ZCL_Z_EWM_INVENTORY_09_DPC IMPLEMENTATION.


  method /IWBEP/IF_MGW_APPL_SRV_RUNTIME~CREATE_ENTITY.
*&----------------------------------------------------------------------------------------------*
*&  Include           /IWBEP/DPC_TEMP_CRT_ENTITY_BASE
*&* This class has been generated on 18.09.2025 13:20:07 in client 100
*&*
*&*       WARNING--> NEVER MODIFY THIS CLASS <--WARNING
*&*   If you want to change the DPC implementation, use the
*&*   generated methods inside the DPC provider subclass - ZCL_Z_EWM_INVENTORY_09_DPC_EXT
*&-----------------------------------------------------------------------------------------------*

 DATA inventoryadjustm_create_entity TYPE zcl_z_ewm_inventory_09_mpc=>ts_inventoryadjustmentitemn.
...
*-------------------------------------------------------------------------*
*             EntitySet -  InventoryAdjustmentItemNSet
*-------------------------------------------------------------------------*
     WHEN 'InventoryAdjustmentItemNSet'.
...
*&-----------------------------------------------------------------------------------------------*
 ...
 DATA inventoryadjustm_get_entity TYPE zcl_z_ewm_inventory_09_mpc=>ts_inventoryadjustmentitemn.
 DATA lv_entityset_name TYPE string.
...
*&*   generated methods inside the DPC provider subclass - ZCL_Z_EWM_INVENTORY_09_DPC_EXT
*&-----------------------------------------------------------------------------------------------*
 DATA inventoryadjustm_get_entityset TYPE zcl_z_ewm_inventory_09_mpc=>tt_inventoryadjustmentitemn.
 DATA lv_entityset_name TYPE string.
...
*&-----------------------------------------------------------------------------------------------*
 ...
 DATA inventoryadjustm_update_entity TYPE zcl_z_ewm_inventory_09_mpc=>ts_inventoryadjustmentitemn.
 DATA lv_entityset_name TYPE string.
 DATA lr_entity TYPE REF TO data. "#EC NEEDED
...
METHOD inventoryadjustm_create_entity.
  ...
  "--- local copy of incoming payload (segw generated structure) ---
  DATA: ls_input     TYPE zcl_z_ewm_inventory_09_mpc=>ts_inventoryadjustmentitemn.
  ...
  "--- quantity as internal numeric type used for PI count ---
  DATA: lv_qty       TYPE /scwm/de_quantity.

  "--- user string (from either UNAME or PROCESSOR) ---
  DATA: lv_user      TYPE string.
  DATA: lv_cntqty_str TYPE string.
  ...
  CONSTANTS: lc_act       TYPE /lime/pi_active        VALUE 'A',
             ...
             lc_altme_def TYPE meins VALUE 'EA'. " default UoM
...
*--------------------------------------------------------------------
* 2. Determine user value robustly (handles UNAME or PROCESSOR)
*--------------------------------------------------------------------
  " Try UNAME then PROCESSOR (some models use different field names)
  FIELD-SYMBOLS: <fs> TYPE any.
  ASSIGN COMPONENT 'UNAME'  OF STRUCTURE ls_input TO <fs>.
  IF <fs> IS ASSIGNED.
    lv_user = <fs>.
  ELSE.
    ASSIGN COMPONENT 'PROCESSOR' OF STRUCTURE ls_input TO <fs>.
    IF <fs> IS ASSIGNED.
      lv_user = <fs>.
    ELSE.
      lv_user = sy-uname. " fallback
    ENDIF.
  ENDIF.
...
*--------------------------------------------------------------------
* 4b. Prepare converted quantity (CNTQTY)
*     - We accept CNTQTY coming as string (e.g. "2.000") in OData
*     - Clean it and convert to /SCWM/DE_QUANTITY
*--------------------------------------------------------------------
  " Get CNTQTY value robustly (some models expose as CHAR, others as P)
  CLEAR: lv_cntqty_str, lv_qty.
  ASSIGN COMPONENT 'CNTQTY' OF STRUCTURE ls_input TO <fs>.
  IF <fs> IS ASSIGNED.
    " if incoming component is numeric already, convert to string for normalization
    TRY.
        lv_cntqty_str = |{ <fs> }|.
      CATCH cx_root.
        lv_cntqty_str = ''.
  ...
  " Normalize decimal separator and whitespace
  IF lv_cntqty_str IS NOT INITIAL.
    " replace comma with dot (3rd parties may send locales)
    REPLACE ALL OCCURRENCES OF ',' IN lv_cntqty_str WITH '.'.
    CONDENSE lv_cntqty_str NO-GAPS.
  ENDIF.

  " Convert to internal quantity (/SCWM/DE_QUANTITY) safely
  IF lv_cntqty_str IS NOT INITIAL.
    TRY.
        lv_qty = CONV /scwm/de_quantity( lv_cntqty_str ).
      CATCH cx_root.
        " fallback: try to convert integer portion, else zero
        TRY.
            lv_qty = CONV /scwm/de_quantity( lv_cntqty_str ).
          CATCH cx_root.
            lv_qty = 0.
    ...
    ENDTRY.
  ELSE.
    " If nothing provided default to 0
    lv_qty = 0.
  ENDIF.


*--------------------------------------------------------------------
* 5. CREATE PI Document (one line)
*--------------------------------------------------------------------
  ...
  " OWNER / ENTITLED
  IF ls_t300_md-entitled IS NOT INITIAL.
  ...
  " commit after create to get doc persistent
...
*--------------------------------------------------------------------
  ...
  <ls_res1>-data-hu_item-huident  = ls_input-vlenr.
  ...
  <ls_res2>-data-hu_parent-huident  = ls_input-vlenr.
  ...
  " Quantity line - use converted numeric lv_qty and ensure unit
  APPEND INITIAL LINE TO <ls_res2>-t_quan ASSIGNING FIELD-SYMBOL(<ls_quan>).
  IF <ls_quan> IS ASSIGNED.
    <ls_quan>-qan_status       = 'C'.
    <ls_quan>-entered_quantity = lv_qty.
    " ALTME: use incoming or default
    ASSIGN COMPONENT 'ALTME' OF STRUCTURE ls_input TO <fs>.
    IF <fs> IS ASSIGNED AND <fs> IS NOT INITIAL.
      <ls_quan>-entered_unit = <fs>.
    ELSE.
      <ls_quan>-entered_unit = lc_altme_def.
    ENDIF.
  ENDIF.
...
*--------------------------------------------------------------------
  ...
  er_entity-processor = lv_user.
  er_entity-vlenr     = ls_input-vlenr.
  er_entity-matnr     = ls_input-matnr.
  er_entity-cntqty    = lv_cntqty_str. " return original string representation
